...
namespace Assets.Scenes.GamingStudy0328
{
    public class Player : Creature
    {
        ...
        public Player(GameObject imageGO) : base(imageGO, "Player")
        {
            ...
            MagicBarGO = Object.Instantiate(GameManager.instance.GameBoard.Resource_MagicBar, SpriteGO.transform);
            BloodBarGO = Object.Instantiate(GameManager.instance.GameBoard.Resource_BloodBar, SpriteGO.transform);
        }

        internal void Collect(Floor[,] floors)
        {
            GameManager.instance.GameBoard.DisplayText($"---拾取---");
            if (floors[X, Y].Items.Count == 0)
            {
                GameManager.instance.GameBoard.DisplayText($"{Name}捡了一把空气");
                return;
            }
            for (int i = floors[X, Y].Items.Count - 1; i >= 0; i--)
            {
                ...
                GameManager.instance.GameBoard.DisplayText($"{Name}拾取了{item.Name},{item.ItemType}");
        ...
        {
            GameManager.instance.GameBoard.DisplayText($"---查看---");
            foreach (var item in floors[X, Y].Items)
            {
                item.Display(GameManager.instance.GameBoard);
            }
            foreach (var creature in floors[X, Y].Creatures)
            {
                if (creature.IsPlayer)
                    continue;
                creature.Display(GameManager.instance.GameBoard);
            }
        }

        internal void UseFastItem(string v)
        {
            GameManager.instance.GameBoard.DisplayText($"---使用道具---");
            var fastItem = FastItems.FirstOrDefault(c => c.Key == v);
            if (fastItem == null)
            {
                GameManager.instance.GameBoard.DisplayText($"{Name}喝了口空气");
                return;
            }
            var buff = VLDictionaries.BuffDic[fastItem.Item.ItemType];
            Buffs.Add(buff.Key, buff.Value);
            GameManager.instance.GameBoard.DisplayText($"{Name}使用了{buff.Key},持续({ buff.Value})回合");

            //二步走
            //操作 界面对象
            //操作 逻辑对象
            GameManager.Destroy(fastItem.Item.SpriteGO);//界面
